...
def create_app(test_config: dict | None = None) -> Flask:
    ...
    def datetime_label(value: str) -> str:
        try:
    ...
    @login_required
    def document_editor(document_id: int):
        ...
        is_owner = int(document["owner_id"]) == current_user_id()
    ...
    @login_required
    def share_document(document_id: int):
        document = database.get_document(
            app.config["DATABASE"], document_id, viewer_id=current_user_id()
        )
        if document is None:
            abort(404)
        if int(document["owner_id"]) != current_user_id():
            abort(403)
    ...
    def forbidden(_error):
        return render_template("error.html", title="Access denied", message="You do not have permission to view that document."), 403

    @app.errorhandler(404)
    def not_found(_error):
        return render_template("error.html", title="Not found", message="The page you requested does not exist or is not shared with you."), 404
